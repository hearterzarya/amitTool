...
const getSheetSideClasses = (side: "top" | "bottom" | "left" | "right" = "right") => {
  const base = "fixed z-50 gap-4 glass-strong p-6 shadow-soft-xl border-l border-border/50 transition ease-in-out data-[state=open]:animate-slide-in-right data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500"
  const sides = {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-4 top-4 rounded-lg glass p-2 opacity-70 ring-offset-background transition-all hover:opacity-100 hover:glass-strong focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-5 w-5" />
